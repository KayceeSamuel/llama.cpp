...
// The int8 codebook: NF4 levels on a 1/127 grid. The CUDA path uses these so
// it can dp4a; the CPU reference uses the same values so the two are
// bit-identical. Measured cost of the rounding to int8: 0.088714 -> 0.088922
// on kurtosis-1.4 data, 0.101801 -> 0.102338 with 16-sigma outliers.
...
//
// Fitted on a MIX of outlier-free and 16-sigma-outlier data. Do not re-fit on
// outlier-free data alone. A version that did spanned only 0.3355 to 1.0 and
// failed badly whenever a superblock contained an extreme weight: the other
// sub-blocks then have ratios near 0.06, clamp to the 0.3355 floor, and their
// weights collapse into the lowest NF4 levels. Measured 0.139 against 0.101
// for this codebook on outlier data. Real transformer weights measured max/std
// of 15.4 and 16.8, so that regime is the normal case, not an edge.
//
// A model with a different outlier profile may want a different floor. The
// diagnostic is the distribution of sc[] indices: significant mass in bin 0
// means the floor is too high for that model.
...
// ---------------------------------------------------------------- fp16
// ggml's conversion, not a local copy: better tested, and possibly
...
void quantize_row_nf4dq_ref(const float * restrict x, block_nf4dq * restrict y,
    ...
    for (int64_t i = 0; i < nb; i++) {
        ...
        // Fold the codebook's /127 into the stored scale, as IQ4_NL does, so
        // neither the CPU nor the CUDA decode path needs a division.
        ...
        for (int s = 0; s < NF4DQ_NSUB; s++) {
            ...
            // Weights are quantised against the reconstructed sub-block scale,
            // for the same reason as above: quantise against what the decoder
            // will use. d_eff already carries the /127, so the reconstruction
            // is I8[idx] * d_eff * SCALE[si] and the grid being quantised
            ...
            for (int j = 0; j < NF4DQ_SUB; j++) {
                ...
                // ggml packing: within a 32-element sub-block, byte b holds
                // element b in the low nibble and element b+16 in the high
...
void dequantize_row_nf4dq(const block_nf4dq * restrict x, float * restrict y,
    ...
    for (int64_t i = 0; i < nb; i++) {
        ...
        const float d = fp16_to_fp32(xb->d);   // already carries the /127
        for (int s = 0; s < NF4DQ_NSUB; s++) {
            ...
            for (int j = 0; j < NF4DQ_SUB; j++) {
                ...
                // I8 grid, matching vec_dot_nf4dq_q8_1 exactly.
...
// they align one to one: sub-block s pairs with q8 block s. This is the same
// alignment IQ4_NL relies on, and it is why Q8_0 is the right vec_dot_type.
//
